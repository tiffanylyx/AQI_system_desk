...
var barwidth_org = screen.width*0.08
var padding_bar_org = screen.width*0.043
var barwidth = barwidth_org
var padding_bar = padding_bar_org
var padding_text = 100
...
Promise.all([
  ...
  const functionsArray = [
      ...
      { func: move_bar, args: [date, data_for_day, info,60] },
...
function raw_number(data,info){
  len = 8000
  barwidth = barwidth_org
  padding_bar = padding_bar_org
  ...
  .attr('x',0)
  .attr("dy", 0.5)
  .attr('y',-height*0.38).style("text-anchor","middle")
  ...
  text_box = explain_group.insert('rect', 'text') // Insert rectangle before the text element
      .attr('x', -text_length/2-padding_text/2)
...
info_group.selectAll("*").remove()
  ...
  .attr('y', function() {
    ...
    return parseInt(d3.select(this).attr('y')) + distance;})
      ...
      labels1.attr("x", barwidth / 2).style('font-size',16)
